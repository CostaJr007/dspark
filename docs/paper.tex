...
\begin{equation}
\text{Conf}(b) = 1.0 - (0.40 \cdot \mathcal{H}_{\text{cyclo}}(b)) - (0.25 \cdot \mathbf{1}_{\text{complex}}(b)) - (0.20 \cdot \mathbf{1}_{\text{mutating}}(b))
\end{equation}
where $\mathcal{H}_{\text{cyclo}}(b) = \min(1.0, \frac{\text{CycloComplexity}(b) - 1}{15})$, $\mathbf{1}_{\text{complex}}(b)$ indicates nested closures or unsafe pointers, and $\mathbf{1}_{\text{mutating}}(b)$ indicates global state mutation. Blocks with $\text{Conf} > 0.88$ are approved locally at zero API cost.
...
\begin{enumerate}
    ...
    \item \textbf{Anchor Pivot Selection}: Trajectories are ranked by win-rate mass; the top $k = \operatorname{clamp}(1, \lfloor N/2 \rfloor, k_{\text{req}})$ candidates are selected as pivots $\mathcal{P}$.
...
\begin{enumerate}
    \item The execution traceback is parsed into a structured counterexample tuple $\mathcal{C} = \langle \text{test}, \text{line}, \text{traceback\_tail}, \text{expected}, \text{actual} \rangle$.
