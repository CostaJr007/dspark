    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "DSpark is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "DSpark may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("DSpark"), "full badge: {full:?}");
        assert!(inline.contains("DSpark"), "inline badge: {inline:?}");
