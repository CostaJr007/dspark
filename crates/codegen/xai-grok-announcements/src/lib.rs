...
pub fn filter_expired_at(
    ...
    announcements
        .into_iter()
        .filter(|a| !is_expired_at(a, now) && !is_vendor_promo(a))
        .collect()
}
...
fn is_vendor_promo(a: &RemoteAnnouncement) -> bool {
    ...
    hay.contains("grok") || hay.contains("spacexai") || hay.contains("x.ai")
}
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("GROK_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => {
                let list: Vec<_> = list.into_iter().filter(|a| !is_vendor_promo(a)).collect();
                return Some(list);
            }
            Err(e) => {
    ...
    remote_announcements.map(|list| {
        list.into_iter()
            .filter(|a| !is_vendor_promo(a))
            .collect()
    })
}
