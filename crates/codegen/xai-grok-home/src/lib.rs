//! Single source of truth for the DSpark home directory: `$DSPARK_HOME`,
//! `$GROK_HOME`, or `<home>/.dspark`. Shared by `xai-grok-config` and
//! `xai-fast-worktree`. Default is `.dspark` so this binary never shares
//! state with the official `grok` install (`~/.grok`).
//!
...
/// `<home>/.dspark`, canonicalized via `dunce` (not `std::fs::canonicalize`,
...
/// `$DSPARK_HOME` or `$GROK_HOME` verbatim when non-empty, else `<home>/.dspark`.
...
/// Resolve home from the environment (fresh, no cache); `None` if neither resolves.
pub fn resolve_grok_home() -> Option<PathBuf> {
    resolve_grok_home_from(
        std::env::var_os("DSPARK_HOME")
...
/// The default `<home>/.dspark`, used when no home env is set.
...
mod tests {
    ...
    fn empty_env_falls_through_to_os_home() {
        ...
        assert_eq!(
            resolved,
            Some(dunce::canonicalize(tmp.path()).unwrap().join(".dspark"))
        );
    }
