...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        // `/dev/stdout` and `/dev/null` are Unix-only; Windows protoc cannot
        // open them, so write both artifacts to temp files.
        for proto in protos {
            let stamp = format!(
                "dspark-protoc-{}-{}",
                std::process::id(),
            ...
            let dep_path = tmp.join(format!("{stamp}.d"));
            let desc_path = tmp.join(format!("{stamp}.pb"));
            ...
            let rem = first_line.strip_prefix(&prefix).or_else(|| {
                // protoc may emit forward slashes even on Windows.
                first_line.strip_prefix(&format!("{}:", desc_path.to_string_lossy().replace('\\', "/")))
            }).with_context(|| {
                format!("protoc dependency file must start with descriptor path: {output:?}")
            })?;
