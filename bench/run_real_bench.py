...
def chat(model: str, messages: list[dict], temperature: float, provider: str) -> tuple[str, dict]:
    if provider == "openai":
...
# --------------------------------------------------------------- PPT tournament
def ppt_compare(a_code: str, b_code: str, spec: str, rng_salt: int) -> bool:
    ...
    text, _ = chat("gpt-4o-mini", [{"role": "user", "content": prompt}], 0.0, "openai")
...
def ppt_run(codes: list[str], spec: str) -> tuple[int, list[tuple[int, float]]]:
    ...
    def match(i: int, j: int) -> None:
        a_won = ppt_compare(codes[i], codes[j], spec, i * 31 + j)
...
# ------------------------------------------------------------------ generation
def generate(spec: str, model: str, provider: str, temperature: float) -> tuple[str, float]:
    prompt = (
...
def refine_with_flagship(spec: str, code: str, failure_tail: str) -> str:
    prompt = (
...
# ----------------------------------------------------------------------- pilot
def pilot_task(task: dict, out_rows: list) -> None:
    tid = task["id"]
    log(f"\n=== {tid} ({task['kind']}) ===")

    # ---- config B: cheap-only single shot
    code_b, _ = generate(task["spec"], "gpt-4o-mini", "openai", 0.2)
    ...
    code_a, _ = generate(task["spec"], ds_model, "deepseek", 0.2)
    ...
    for i, temp in enumerate(DRAFT_TEMPERATURES):
        code, _ = generate(task["spec"], "gpt-4o-mini", "openai", temp)
    ...
    winner_idx, rates = ppt_run(drafts, task["spec"])
    tie = is_tie(rates)
    ...
    if not ppt_passes:
        escalated = True
        fixed = refine_with_flagship(task["spec"], final_code, draft_results[winner_idx]["tail"])
        final_code = fixed
...
def summarize(rows: list[dict]) -> None:
    ...
    log("=" * 64)
    labels = [("B", "cheap-only (gpt-4o-mini)"), ("A", "flagship-only (v4-flash)"),
              ("V", "verify-all-first-pass"), ("C*", "best-of-3 RANDOM (expected)"),
              ("T", "PPT pick, no escalation"), ("D", "FULL tiered + v4-flash")]
    for key, name in labels:
        p = c_exp() if key == "C*" else pct(key)
        lo, hi = wilson_ci(p, n)
        bar = "#" * int(p * 40)
        log(f"  {name:<28} {p*100:5.1f}%  [{lo*100:.1f}-{hi*100:.1f}]  {bar}")
    ...
    log(f"Q2  +v4-flash      : {m_q2:+.1%} pts  | escalations={len(esc)} "
...
def main() -> None:
    ...
    rows: list[dict] = []
    log(f"Pilot: {len(tasks)} tasks | caps: ${BUDGET_CAP_USD}/provider | "
        f"N={N_DRAFTS} k={K_PIVOTS_REQUESTED}")
    try:
        for t in tasks:
            pilot_task(t, rows)
